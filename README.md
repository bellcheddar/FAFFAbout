...
- [x] **Phase 2: rebuild the corpus after the `nan` fix.** `float('nan')` is truthy and is not None, so it defeated every guard in the builder and reached **10,391 training targets** (8.7%): 8,304 reading `Tag: nan, cleaved with nan.` and 4,609 reading `Host: nan, following the precedent in this cluster.` A `nan` in a prompt teaches forecasting from absent features, which is legitimate; a `nan` in a target teaches the model to write it as a protocol choice. Fixed with `present()`, unit-tested against all eight NaN variants plus `0` and `0.0` (real values for `cys_count` and `tm_helices`). The corpus on disk is still the defective one, so round 01 trained on it
...
- [ ] **Phase 3: deduplicate the corpus?** 78.8% of `gate_judgement`, 88.0% of `failure_attribution` and 77.2% of `construct_recommend` training records are **byte-identical** to another record; one completion appears **527 times**. `orthologue_ranking` is 0% duplicated because it names identifiers, which is the clue: the other tasks generate from a few templates over coarse feature buckets, so distinct proteins collapse onto the same text. Deduplicating would cut the corpus by more than half and change the budget, the mix and the splits, so it is a design decision rather than a fix
- [ ] **Phase 3: LoRA.** Round 01 (abandoned at iteration 570) trained on a corpus whose targets contained `nan` 10,391 times; round 02 (stopped at iteration 10) on one whose forecasts were filled 37% from the target's own outcome. **Round 03 is running on a corpus verified free of both.** The learning rate is 2e-5, not the specification's 1e-4, which diverged 0.234 → 4.831 → 13.217. Everything launched from an agent session inherits **nice 5**, so it crawls until `sudo renice -n 0 -p <pid>` is run in a real Terminal
...
- [ ] **Phase 3: grade the narratives.** 40 cases in `eval/generative_review.md`. An adapter now exists, but it was trained on the defective corpus, so grading it would measure the defect. Worth doing after the rebuild
- [ ] **Phase 3: serve the adapter by the right name.** `mlx_lm.server` registers `--adapter-path` under the literal key `default_model` and looks it up by the *requested* model name, so asking for the base model silently serves a second, un-adapted copy. That scored plain Llama for a whole evaluation while every name-based check passed. Fixed in `evaluate_round.sh` and `app/llm.py`; the lesson is to verify by **output** (adapted prose is terse and in-register, base prose opens "I can simulate a protein attrition forecast...") rather than by identity
...
- [x] **Phase 4: deploy tooling.** `deploy/deploy.sh` (dry run unless given `--go`, refuses to ship a booster without its schema, never `--delete`s over the data tree) and `deploy/faffabout.nginx.conf`
- [ ] **Phase 4: deploy.** faffabout.mdeller.com and an entry on the mdeller.com launcher. The payload is 1.9 GB, of which the MMseqs2 index is 1.7 GB and worth 0.4 s per search against 4.1 s without it
