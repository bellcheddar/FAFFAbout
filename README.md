...
- [ ] **Phase 2: rebuild the corpus after the `nan` fix.** `float('nan')` is truthy and is not None, so it defeated every guard in the builder and reached **10,391 training targets** (8.7%): 8,304 reading `Tag: nan, cleaved with nan.` and 4,609 reading `Host: nan, following the precedent in this cluster.` A `nan` in a prompt teaches forecasting from absent features, which is legitimate; a `nan` in a target teaches the model to write it as a protocol choice. Fixed with `present()`, unit-tested against all eight NaN variants plus `0` and `0.0` (real values for `cys_count` and `tm_helices`). The corpus on disk is still the defective one, so round 01 trained on it
...
- [ ] **Phase 3: LoRA.** Round 01 ran for eight hours and wrote its first checkpoint at iteration 500 (val loss 0.049, best train 0.041, no divergence after the learning rate was cut from the specification's 1e-4 to 2e-5, which had diverged 0.234 → 4.831 → 13.217). Two problems outrank finishing it: the corpus it trained on carries the `nan` defect above, and the process sits at **nice 5** because anything detached from this session is demoted, so it crawled at roughly 200 s/iter against 15 s/iter healthy. `sudo renice -n 0 -p <pid>` in a real Terminal fixes the second; the first needs a rebuild and a restart
